...
- **Aug 29, one shared browser, tiles as tabs.** The live-tile transport is redesigned: every board tile is now a window inside one shared Cloudflare Browser Rendering session instead of a whole browser of its own, and the concurrency cap counts tabs (six) rather than browsers (two). This is what fixed the dead "No frames received" tiles: a mission's Jarvis close opens four retailer pages at once, which could never fit two-browser capacity, while the account itself only allows three browsers and research needs the other two. Verified live against Cloudflare before shipping: multiple WebSocket clients per session, `newWindow: true` per tile (same-window background tabs stop painting, so this is load bearing), reattach inside the grace period preserving page state, and single-tab close leaving neighbours streaming. Research session creation now retries with bounded backoff when Cloudflare refuses at the account cap, because a refusal there is a collision with a sibling branch, not a fault.

- **Aug 29, the demo store is Thuma, and a prepared cart is the closing tile.** `CARDEA_SHOPIFY_STORE_DOMAIN` now points at `www.thuma.co` (verified end to end live: "bed frame" search, product detail, cart preparation at $895.00, checkout handoff URL), so the demo prompt about buying a bed lands on the reliable storefront MCP path with real prices instead of bot-walled retailer scraping. Reversible with one env var. Planner-facing Shopify capability descriptions now name the configured store so the model can judge whether a goal's product category plausibly belongs there. The board's Jarvis close now ranks a prepared cart's checkout handoff URL (or a composed cart permalink) as the primary tile, so a buying mission visibly ends on the merchant's own cart in the live browser.

- **Aug 29, the Shopify store is a default, not a boundary.** Every Shopify capability now accepts an optional `store` input: a bare storefront hostname the planner may pass to act on any Shopify store that mission evidence named, with `CARDEA_SHOPIFY_STORE_DOMAIN` remaining only the default. The override value comes from a model, so it is validated harder than the env var (no IP literals, no non-public suffixes, refused rather than defaulted on failure), and the request it steers is only ever an https POST to Shopify's fixed MCP path. Verified live in one process: default call answered by Thuma, overridden call answered by Allbirds. Also verified this session: the UCP successor surface works end to end through Cardea's own deployed agent profile (search, detail, cart, checkout handoff on thuma.co), so the Aug 31 legacy sunset does not threaten production, which auto-selects UCP because the profile URL is set.

- **Aug 29, research now ranks its own results, so "first" actually means "best."** Root-caused live, not guessed: ran Cardea's real `runResearch` against `ikea.com` for "twin bed frame ikea" and it found two exact, priced products (NEIDEN $59, VIHALS $189) every time, proving the browsing and product-hop logic already worked. The bug was downstream. `runResearch` orders results in crawl order (search hits first, the one or two product-hop pages appended last), and two consumers both silently assume "first" means "best": the board opens the first successful result as the live tile, and `execute-node.ts`'s upstream-evidence digest hard-caps each dependency at 3,400 characters. Verified precisely: the two real product URLs landed at character offset 3,996+, so the consolidation model that writes the mission's final recommendation never saw them, only a generic category page, an eBay error page, and an unrelated assembly-service listing. `rankResearchResults` (`web-research.ts`) now stable-sorts a run's results (priced product page, then priced-or-product, then plain read, then failure) before returning them, so both consumers get the useful evidence first without either one needing its own notion of relevance. Verified live again post-fix: both product URLs now survive the same 3,400-character cap.

- **Aug 29, a node card's "recorded" label was eating one of its three lines.** `NodeCard`'s work note used `-webkit-line-clamp: 3` on a box whose own first child was the "RECORDED" label; since line-clamp counts rendered lines regardless of which element they belong to, the label silently spent one of the three lines on itself, and real findings were losing their last line, sometimes the one line naming the exact product and price. The label now renders outside the clamped box (`node-card.tsx`, `.workBlock` in `node-card.module.css`) so all three clamped lines belong to the actual finding.

- **Aug 30, the finish demo's own scroll was eating its buttons, and a fixed noise layer was rendering solid black on mobile scroll.** Two unrelated real bugs, both live-verified. First: `ConciergeClose`'s bubble caps at a real, board-appropriate height with internal scroll; reused unmodified on the landing page's finish demo, that cap (240px, then found to need 350px against measured content) clipped "the receipts" and the remember button below the fold of a demo nobody would think to scroll, and DebriefCard's own landing-only box was worse, 230x310 fixed, cutting the verdict sentence off before a reader reached it. Landing-only overrides now size to this demo's own fixed, measured content (bubble to 350px, brief to 280x330 desktop clear of the bubble, mobile uncapped since it stacks in normal flow with no collision risk) rather than inheriting the real board's operator-facing scroll behavior. Also fixed at the source, so it is not landing-only: `.receiptsToggle`'s own `margin-top` stacked with `.quietRow`'s flex `gap` whenever the row wrapped to two lines, doubling the space above the remember button on both the landing demo and the real board. Second, unrelated: `body::before` (the whole-page 9% noise grain) was `position: fixed` with `mix-blend-mode: multiply` at z-index 1000, and on mobile Chrome that specific combination intermittently composited against solid black across large scrolled regions instead of the page beneath it, reproduced live on the deployed site and confirmed by disabling it. It is `position: absolute` off a newly `position: relative` body now, sized to the document instead of the viewport, scrolling with the page instead of repainting every frame; a tiled pattern looks identical either way.

- **Aug 30, the constellation vignette shows six systems, not four, and "canvas ui" sits behind the finish demo.** `ConstellationVignette` orbited composio, supermemory, chrome, and openai; cloudflare and shopify, promoted to live integrations earlier this session, were absent from this one truth surface even though the caption claimed to describe "connected apps" without depicting the two apps it actually named (Gmail, Calendar). Now six real marks orbit in a hexagon (openai, chrome, cloudflare, shopify, supermemory, composio), and the caption states what is actually shown rather than naming Gmail and Calendar, which no icon here represents (Composio is the real system behind that pipe; Cardea does not fabricate a logo it has no license for, unchanged from the earlier ReceiptsVignette decision). Separately, the finish demo gained three unlabeled, low-contrast ghost card silhouettes behind the tile, bubble, and brief, pure abstraction and never fabricated content, so the demo reads as sitting on a board with more on it rather than floating in empty dotted space, especially on mobile once the pieces stack and leave real empty margin.

- **Aug 30, the "Keep it" pill was wrapping onto two lines.** `.vig-remember`'s two children (the question text and the inert "Keep it" pill) both had default flex-shrink, so a narrow column squeezed both equally and the short action label broke into "Keep" over "it" instead of the longer question text absorbing the squeeze. The pill now never shrinks or wraps; the question text does, since it is long enough to safely.

- **Aug 30, the "working"/"recorded" label lost its inset, and the finish demo's ghost cards read as a bug.** The Aug 29 fix that moved `.workLabel` out of the clamped `.workNote` box (to stop line-clamp eating a real evidence line) only gave the new `.workBlock` wrapper a top margin, not the horizontal padding the label used to inherit from sitting inside `.workNote`; the label rendered flush against the card's inner edge. `.workLabel` now carries its own `padding: 0 10px`, matching `.workNote`'s. Separately, the three decorative ghost-card silhouettes added to the finish demo the same day (meant to read as "more board behind this") instead read as an unexplained stray empty card, confirmed live via screenshot. Removed rather than tuned: the stage's own dotted board-material background already carries that job. Also deleted a stale code comment in `globals.css` describing the glass-hero exception, superseded same-day by the no-glass decision and no longer describing anything live.

- **Aug 30, first batch off the parked-features backlog: Supermemory tenant scoping, rating extraction, and a Shopify auto-detection bridge.** Three of nine backlog items executed (`docs/PARKED_FEATURES.md`), chosen because they were small, fully verified against current code, and did not depend on an unreachable live credential. Supermemory's `containerTag` now keys on `tenantId` (building `tenant:{tenantId}`) instead of `userId`, matching the boundary the rest of persistence already authorizes by; verified no pre-existing promoted memories needed migrating. Page reads now extract ratings and review counts alongside prices (`PAGE_READ_EXPRESSION`), verified against a real Thuma product page before writing the regex, then proven live end to end pulling real signals off Forbes and Mattress Clarity. Research results now auto-detect a Shopify storefront on the single most specific top result (a product-shaped URL with an observed price or rating) and fold in real structured Shopify catalog data via a direct capability call, no separate detection probe, `products.json` was tried first and rejected after it failed live against Thuma's own custom storefront front-end despite Thuma's real Shopify MCP working fine. One real bug caught by that same live check before shipping: the Shopify enrichment was being silently truncated away by two independent excerpt-length caps that assumed the base excerpt had spare room, which it usually does not; fixed in both places with a regression test. Also shipped in the same batch: an expanded, accurate Shopify section in the README (was one buried bullet, undersold what the spike ticket had already verified).

- **Aug 30, product-hop selection gets a model's judgment when the regex alone cannot discriminate.** `selectProductLinksSmart` wraps the existing deterministic `selectProductLinks`: below the hop budget, identical behavior, no model call, no cost change. Above it, several product-shaped candidates the regex has no way to rank against a stated budget or size, one bounded, low-effort model call picks between them, structured-output validated against the real candidate hrefs, falling back to the plain order on any failure (no key, timeout, bad output), and explicitly respecting a "none of these fit" judgment rather than opening something anyway. One real bug caught live before shipping, not assumed away: the existing 2-per-host cap, correct for the "just open pages" path, was also silently squeezing the pool a model gets to judge down to two whenever every candidate came from one store's own category page, which real stores do constantly, a separate, wider per-host limit now governs the collection pass specifically. Live-verified against real Thuma candidates: correctly identified the one true match for a stated budget-and-category constraint out of five candidates that all passed the same regex filter equally.

- **Aug 30, memory learns from a second door: any resolved approval where a person picked an alternative, not only a buying mission's close.** The blocker was always the UI home, not the plumbing, `/api/memory/promote` already took a free `source` string, so no server change was needed. Reused `ApprovalCard`'s own existing anatomy instead of building the fuller sticky-note surface DESIGN.md describes (still parked): the card now distinguishes a note that came from clicking a listed alternative from one typed freehand, and only the former gets a "Remember this choice" checkbox, since a plain accept carries no new information and a freeform edit has no clean "chose X over Y" fact to extract. Live-verified in a real browser via a temporary debug route rendering the card directly (deleted after): checkbox appears only after picking an alternative, and confirming fires the real promote call with the correct fact text alongside the real resolve call.

- **Aug 30, the context wallet gets a WebMCP path, scoped to what the wallet actually is.** The parked entry originally imagined a `dataSource`-level `update_wallet_selection`, mirroring `update_mandate`. Re-reading `use-wallet.ts` before touching anything showed that was the wrong shape: the wallet is pure client-side React state, entirely separate from mission/mandate data except for a one-time copy into `selectedContextCardIds` at `createMission`, and `updateMandate` still carries that field through unchanged after a mission opens (reconfirmed, not assumed), so there is no mission-level selection to mutate today, only a pre-mission one. Shipped instead: a `toggle_wallet_pass` WebMCP tool that calls the wallet's own `toggle(id)` directly, the same tier as `focus_node`/`open_takeover` (a pure client action, freely callable, no new authority granted), plus `inspect_canvas` now reports every starter pass with its live selection and loaded amount so an agent can see what exists before proposing a change. Live-verified against the real running `/app` board by registering a fake `document.modelContext` (Chrome's real WebMCP API is not present in stock Chromium/Playwright) and driving the actual registered tools: all 13 tools present, wallet read correctly, a real toggle round-tripped through `inspect_canvas` after a render tick, and an unknown pass id refused cleanly without mutating anything.

- **Sept 1, the pre-final sweep: five parallel audits (security, backend correctness, frontend vs the design docs, the WebMCP surface, config hygiene), every confirmed finding fixed the same day.** The load-bearing ones: the approval-resolve route now verifies the path approval belongs to the mission the caller was authorized against, closing a guest/judge service-role path that could settle another tenant's approval by id; the shared live-browser ledger is namespaced per caller identity so one session can never reattach to or close another's tab, and both browser routes plus approval-resolve gained rate limits. The retry/replay contract in `runExecuteNode` was stricter in Postgres than in the in-memory test double, so three real crash-or-double-send paths (re-executing a non-idempotent write after a bookkeeping failure, a replay append with a mismatched payload, completing an already-terminal idempotency row) survived tests that could not see them; the double now models the live RPC semantics and all three are fixed with regression tests. Inngest functions gained `onFailure` handlers so a terminally failed plan or node materializes `mission.failed`/`node.failed` instead of a board stuck "planning" forever, resume continuations are serialized per mission, and `budgetLimits` is clamped at the contract boundary (an empty object used to disable every retry ceiling). Node execution deliberately stays parallel per mission: one proposed serialization was reverted because parallel branches are the product. On the WebMCP surface: agent-created missions now carry the visible wallet selection, budget, and free passage exactly like the human submit path (previously silently dropped, making toggle_wallet_pass a no-op in the agent flow); every validation failure returns the same structured envelope as domain failures, with the offending field named, because Chrome does not document how a thrown execute() surfaces; registration failures log instead of vanishing. On the board: the mandate sheet's free-passage toggle is now a real mandate revision applied at approve time and reset after each create (it previously applied to nothing and leaked into the next mission); landing demos are `inert` (they were fully clickable under aria-hidden, including a "remembered" that saved nothing, a straight never-fabricate violation); the node card's Open button is keyboard-reachable; guests and judges see why the remember affordance is absent instead of nothing.

- **Sept 1, running the flagship mission against production found the thing no test could: two of three retailers refuse to be browsed at all.** The planning rules told the model to search a large retailer's own site and used `site:target.com` as the worked example, so plans aimed squarely at the chains with the most aggressive bot defenses. A real run planned target.com, walmart.com, and wayfair.com; walmart answered "Robot or human?" and wayfair answered "Press & Hold to confirm you are a human", both as HTTP 200 pages with readable text. Nothing recognized them, so challenge copy was recorded as ordinary evidence and handed to consolidation as though it were what the retailer had to say about beds. Cardea still refused to invent a recommendation it could not verify, which is exactly right and is the rule holding under real adversarial conditions, but it could not say why, so the close read as "I found nothing" when the truth was "two sites refused to let me look". Two changes, both measured rather than assumed. Research now detects a challenge page and records it as blocked, requiring both a full challenge phrase and the absence of any observed price so a genuine product page that quotes one of those strings stays evidence, and the summary names how many were blocked. The planning rule now names those three as sites not to aim every step at, prefers the brand's or manufacturer's own store and mid-sized specialists, and asks for a spread so one refusal cannot decide a mission. Verified end to end in production on the identical query: the planner chose ikea.com and zinus.com, and the mission closed with a real top pick (IKEA SLATTUM queen, $149) and a real runner-up (Zinus, from $289), both inside the stated budget, where the previous run had closed with nothing. Do not restore a large-chain example to that rule.

- **Sept 1, verified the tool surface with a real third-party WebMCP client, which found `inspect_canvas` reporting a stale canvas as current.** Every test until now used a `document.modelContext` we injected ourselves; faithful, but ours. `agent-browser` (vercel-labs) launches a real Chrome with WebMCP genuinely enabled, so it is an independent implementation. It discovered all 13 tools with correct descriptions, invoked read and parameterized tools correctly (proving the real client passes params as an object into `execute`, which had been an open question), returned our structured envelopes verbatim, and surfaced both refusal paths (`unknown_wallet_pass`, `invalid_input`) as parseable JSON rather than opaque throws. It also drove the entire journey for real: create_mission, update_mandate, approve_mandate, all persisted. Two findings. First, benign and worth recording: with no session the board redirects to `/signin`, unmounts, and the tools correctly unregister, which is real-Chrome evidence that the abort-based cleanup works. Second, real: with the tab left idle in a headless browser, live updates stopped arriving and `inspect_canvas` reported "planning, 0 nodes" while the server held 4 nodes executing at sequence 11. A reload corrected it, so the data path was fine and only delivery had stalled. For a product whose claim is that the agent sees what the person sees, reporting a stale canvas as current is the one thing that tool must never do, so it now re-reads the mission through the data source's own `adopt` (public, documented as safe to repeat, rebaselines realtime from the sequence it just read) and waits, bounded, for the refreshed snapshot to be committed before answering. Verified against the deployed fix on the same idle-tab scenario: tool and server both report sequence 42. Note this is specific to an idle or throttled tab; a normal visible board updates live, as the earlier end-to-end runs showed.

- **Sept 3, planning latency measured rather than guessed, and the first fix did not fix it.** On the server's own event clock, approve to plan took 80s on one run and 180s on the next for identical input, while every component inside our code was small: model call about 20s, memory retrieval about 1s, persisting the plan about 1s, discovery near zero. The swing between identical runs pointed at scheduling, so the three read-only planning steps were collapsed into one Inngest checkpoint (kept, because fewer round trips is correct regardless) and the Supermemory client was bounded to 8s with no retries (kept, because its defaults of 60s with two retries that also retry on timeout could hold planning for three minutes). Measured again after deploy: 77.4s. That is not a fix, and the collapse ruled per-hop cost out. By elimination the remaining wait is between the event being sent and Inngest first invoking the function, roughly 55s, which is pickup latency on Inngest's side. The most coherent cause is account-wide concurrency on Inngest's free tier: a dozen test missions launched in one hour, each spawning six browsing nodes that run for minutes, hold the slots, so a new plan waits for one. That also explains why it was worse the more we tested. That prediction was then tested and was mostly wrong: after a four minute drain with a single mission, approve to plan was 69s, so the backlog was worth about eight seconds, not forty. The last unmeasured link, the approve POST itself, was then timed at 5.9s, so the send is real but small. Five samples in all: 80, 180, 77, 69, 90 seconds. Inside each, our own work is about 28s (send 5, model 20, memory 1, persist 1). The remaining 45 to 60 seconds sits between the event reaching Inngest and Inngest first invoking the function, which on Inngest's free tier plus Vercel's hobby tier is event pickup and cold start. No change to this codebase moves that floor; the collapse and the memory bound stay because they are correct, not because they fixed it. The only lever that moves it is the platform: a paid Inngest tier and Vercel Pro. Until then the honest expectation, and what the judge instructions should say, is that planning takes one to two minutes and is not hung. Do not reach for more code here; the code is not where the time is.
